...
pub(crate) fn get_eth2_wallet(existing_mnemonic: Option<&[u8]>) -> (Bip39Seed, String) {
...
mod tests {
    ...
    fn it_creates_wallet_with_new_mnemonic() {
        let (wallet, _) = get_eth2_wallet(None);
        assert_ne!(PHRASE_SEED, hex::encode(wallet.as_bytes()));
    }

    #[test]
    fn it_creates_wallet_with_existing_mnemonic() {
        let (wallet, _) = get_eth2_wallet(Some(PHRASE.as_bytes()));
        assert_eq!(PHRASE_SEED, hex::encode(wallet.as_bytes()));
    }
}
